...
def change_password():
    ...
    if form.validate_on_submit():
        # 更新密码
        form.update_password(current_user)
        flash("密码更新成功！", "success")
        return redirect(url_for("index.html"))  # 替换为重定向的目标视图
    return render_template("change_password.html", form=form)  # 渲染表单模板
...
def get_email_captcha():
    ...
    # I/O：Input/Output
    message = Message(subject="世纪在线问答社区注册验证码", recipients=[email], body=f"您的验证码是:{captcha}")
    mail.send(message)
    # 用数据库表的方式存储
